...
ds1 = ds.map(lambda i : _decision_tree.predict(i)) \
	.map(lambda i : (i[5], time.time() - i[6], 'decision'))
#ds1.print()
...
ds2 = ds.map(lambda i : _decision_tree.predict(i)) \
         .map(lambda i : (i[5], time.time() - i[6]))
         
ds3 = ds.map(lambda i : _linear_reg.predict(i)) \
         

ds_to_estimating = ds3.union(ds1)
ds_to_estimating = ds_to_estimating.map(lambda i : _error_estimation.estimation(i)) \
				   .filter(lambda i : True if i is not None else False) \
				   .map(lambda i : (i[5], time.time() - i[6]))

ds_final = ds_to_estimating.union(ds2)
ds_final.print()
'''
env.execute()