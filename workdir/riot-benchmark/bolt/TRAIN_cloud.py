...
def map_func(i):
    start = time.time()
    return (eval(i)['MSGID'], start - eval(i)['timestamp'], start)
...
def map_ec(i):
    start = time.time()
    i = eval(i)
    return (i['V1'], i['V2'], i['V3'], start - i['V4'], start, i['V6'])
    
def map_ect(i):
    start = time.time()
    i = eval(i)
    return (i['V1'], i['V2'], start - i['V3'], start)
    
ds1 = ds1.map(lambda i : map_ec(i)) \
       .rebalance()
ds1 = ds1.map(lambda i : ModelWrite(i)) \
       .map(lambda i : (i[3], time.time() - i[4]))
ds1.print()

ds2 = ds2.map(lambda i : map_ect(i)) \
       .rebalance()
ds2 = ds2.map(lambda i : _decision_tree_train.train(i)) \
        .map(lambda i : ModelWrite(i)) \
        .map(lambda i : (i[3], time.time() - i[4]))
ds2.print()
env.execute()