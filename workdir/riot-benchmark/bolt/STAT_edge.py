...
def map_func_pure_edge(i):
    start = time.time()
    i = eval(i)
    return (i['MSGID'], i['PAYLOAD'], start - i['timestamp'], start)     
    
def map_func_edge_cloud(i):
    start = time.time()
    i = eval(i)
...
def map_to_kafka_format(_input):
    ...
    trans_time = _input[5]
    start = _input[6]
    msg = {'MSGID': msgId, 'SENSORID': sensorId, 'META': meta, 'OBSTYPE': obsType, 'OBSVAL': obsVal, 'TRANS': trans_time, 'START': start}
    return dumps(msg)
...
# SenMLParse
ds = ds.map(lambda i : map_func_pure_edge(i)) \
       .rebalance() \
...
'''
# case edge-cloud
ds = ds.map(lambda i : map_func_edge_cloud(i)) \
       .rebalance() \
       .flat_map(lambda i : _sen_ml_parse.senmlparse(i))
...
ds = ds.map(lambda i : map_func_edge_cloud(i)) \
       .rebalance() \
       .flat_map(lambda i : _sen_ml_parse.senmlparse(i))
ds = ds.map(lambda i : _kalman_filter.filter(i)) \
        .filter(lambda i : True if i is not None else False) \
        .map(map_to_kafka_format, Types.STRING())
ds = ds.map(lambda i : (i.__sizeof__()))
ds.print()
# rest part on the cloud
